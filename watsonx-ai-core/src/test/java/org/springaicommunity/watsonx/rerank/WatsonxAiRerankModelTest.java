...
/**
 * JUnit 5 test class for WatsonxAiRerankModel functionality. Tests rerank model
...
class WatsonxAiRerankModelTest {
	...
	void setUp() throws Exception {
		...
		defaultOptions = WatsonxAiRerankOptions.builder()
			.model("cross-encoder/ms-marco-minilm-l-12-v2")
			.truncateInputTokens(512)
			.build();

		rerankModel = new WatsonxAiRerankModel(watsonxAiRerankApi, defaultOptions, ObservationRegistry.NOOP,
				retryTemplate);
	...
	class ConstructorTests {
		...
		void constructorWithNullApiThrowsException() {
			assertThrows(IllegalArgumentException.class,
					() -> new WatsonxAiRerankModel(null, defaultOptions, ObservationRegistry.NOOP, retryTemplate),
		...
		void constructorWithNullOptionsThrowsException() {
			assertThrows(IllegalArgumentException.class,
					() -> new WatsonxAiRerankModel(watsonxAiRerankApi, null, ObservationRegistry.NOOP, retryTemplate),
		...
		void constructorWithNullRetryTemplateThrowsException() {
			assertThrows(IllegalArgumentException.class,
					() -> new WatsonxAiRerankModel(watsonxAiRerankApi, defaultOptions, ObservationRegistry.NOOP, null),
	...
	class RerankMethodTests {
		...
		void rerankWithValidRequest() {
			...
			List<String> documents = List.of("Machine learning is a subset of AI.", "Cooking recipes are great.",
			...
			WatsonxAiRerankResponse mockResponse = new WatsonxAiRerankResponse("cross-encoder/ms-marco-minilm-l-12-v2",
					"1.0", List.of(result1, result2, result3), LocalDateTime.now(), 15, query);
		...
		void rerankWithNullQueryThrowsException() {
			...
			assertThrows(IllegalArgumentException.class, () -> rerankModel.rerank(null, documents),
		...
		void rerankWithEmptyQueryThrowsException() {
			...
			assertThrows(IllegalArgumentException.class, () -> rerankModel.rerank("", documents),
					"Query must not be null or empty");
		}

		@Test
		void rerankWithEmptyDocumentsThrowsException() {
			assertThrows(IllegalArgumentException.class, () -> rerankModel.rerank("What is AI?", List.of()),
					"Documents must not be empty");
		}

		@Test
		void rerankWithNullDocumentsThrowsException() {
			assertThrows(IllegalArgumentException.class, () -> rerankModel.rerank("What is AI?", null),
					"Documents must not be empty");
		}
		...
		void rerankWithCustomOptions() {
			...
			WatsonxAiRerankOptions customOptions = WatsonxAiRerankOptions.builder()
				.model("custom-model")
				.topN(5)
				.build();
			...
			WatsonxAiRerankResponse mockResponse = new WatsonxAiRerankResponse("custom-model", "1.0", List.of(result),
					LocalDateTime.now(), 5, query);

			when(watsonxAiRerankApi.rerank(any(WatsonxAiRerankRequest.class)))
				.thenReturn(ResponseEntity.ok(mockResponse));

			List<WatsonxAiRerankResponse.RerankResult> results = rerankModel.rerank(query, documents, customOptions);
	...
	class OptionsMergingTests {
		...
		void mergeOptionsWithRuntimeModel() {
			WatsonxAiRerankOptions runtimeOptions = WatsonxAiRerankOptions.builder().model("runtime-model").build();

			String query = "Test query";
			List<String> documents = List.of("Document 1");

			WatsonxAiRerankResponse.RerankResult result = new WatsonxAiRerankResponse.RerankResult(0, 0.9, null);
			WatsonxAiRerankResponse mockResponse = new WatsonxAiRerankResponse("runtime-model", "1.0", List.of(result),
					LocalDateTime.now(), 5, query);

			when(watsonxAiRerankApi.rerank(any(WatsonxAiRerankRequest.class)))
				.thenReturn(ResponseEntity.ok(mockResponse));

			List<WatsonxAiRerankResponse.RerankResult> results = rerankModel.rerank(query, documents, runtimeOptions);
		...
		void mergeOptionsWithRuntimeTopN() {
			...
			WatsonxAiRerankResponse.RerankResult result1 = new WatsonxAiRerankResponse.RerankResult(0, 0.9, null);
			WatsonxAiRerankResponse.RerankResult result2 = new WatsonxAiRerankResponse.RerankResult(1, 0.8, null);
			WatsonxAiRerankResponse.RerankResult result3 = new WatsonxAiRerankResponse.RerankResult(2, 0.7, null);

			WatsonxAiRerankResponse mockResponse = new WatsonxAiRerankResponse("cross-encoder/ms-marco-minilm-l-12-v2",
					"1.0", List.of(result1, result2, result3), LocalDateTime.now(), 10, query);
	...
	class ResponseHandlingTests {
		...
		void handleNullResponse() {
			...
			when(watsonxAiRerankApi.rerank(any(WatsonxAiRerankRequest.class))).thenReturn(ResponseEntity.ok(null));
		...
		void handleResponseWithNullResults() {
			...
			WatsonxAiRerankResponse mockResponse = new WatsonxAiRerankResponse("test-model", "1.0", null,
					LocalDateTime.now(), 5, query);
		...
		void handleResponseWithEmptyResults() {
			...
			WatsonxAiRerankResponse mockResponse = new WatsonxAiRerankResponse("test-model", "1.0", List.of(),
					LocalDateTime.now(), 5, query);
