...
public class WatsonxAiRerankModelIT {
	...
	void setUp() {
		...
		watsonxAiRerankApi = new WatsonxAiRerankApi(BASE_URL, RERANK_ENDPOINT, VERSION, PROJECT_ID, SPACE_ID, API_KEY,
				restClientBuilder, errorHandler);

		WatsonxAiRerankOptions defaultOptions = WatsonxAiRerankOptions.builder()
	...
	void rerankWithSingleDocumentTest() {
		...
		mockServer.expect(requestTo(BASE_URL + RERANK_ENDPOINT + "?version=" + VERSION))
			.andExpect(method(org.springframework.http.HttpMethod.POST))
			.andExpect(header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE))
			.andRespond(withSuccess(jsonResponse, MediaType.APPLICATION_JSON));

		List<String> documents = List.of("Machine learning is a subset of AI.");
		List<WatsonxAiRerankResponse.RerankResult> results = rerankModel.rerank("What is machine learning?", documents);
	...
	void rerankWithMultipleDocumentsTest() {
		...
		List<String> documents = List.of("Machine learning is a subset of artificial intelligence.",
	...
	void rerankWithCustomOptionsTest() {
		...
		List<String> documents = List.of("Machine learning is AI", "Deep learning uses neural networks",
				"Cooking recipes");

		List<WatsonxAiRerankResponse.RerankResult> results = rerankModel.rerank("What is AI?", documents,
				customOptions);
	...
	void rerankWithTopNLimitTest() {
		...
		List<String> documents = List.of("Doc 1 - ML", "Doc 2 - Cooking", "Doc 3 - DL", "Doc 4 - NLP", "Doc 5 - CV");

		List<WatsonxAiRerankResponse.RerankResult> results = rerankModel.rerank("Machine learning topics", documents,
				options);
	...
	void rerankWithEmptyResponseTest() {
		...
		List<WatsonxAiRerankResponse.RerankResult> results = rerankModel.rerank("Unrelated query", documents);
	...
	void rerankWithTruncateInputTokensTest() {
		...
		WatsonxAiRerankOptions options = WatsonxAiRerankOptions.builder().truncateInputTokens(256).build();

		String longDocument = "This is a very long document that contains multiple sentences and paragraphs. "
		...
		List<WatsonxAiRerankResponse.RerankResult> results = rerankModel.rerank("What is this about?",
				List.of(longDocument), options);
	...
	void rerankWithInputTextInResponseTest() {
		...
		List<WatsonxAiRerankResponse.RerankResult> results = rerankModel.rerank("Query text",
				List.of("Original document text"), options);
	...
	void rerankWithDifferentModelsTest() {
		...
		WatsonxAiRerankOptions options = WatsonxAiRerankOptions.builder().model("ibm/rerank-v1").build();

		List<String> documents = List.of("Document A about technology", "Document B about science");

		List<WatsonxAiRerankResponse.RerankResult> results = rerankModel.rerank("Science and research", documents,
				options);
	...
	void rerankPreservesDefaultOptionsTest() {
		...
		List<WatsonxAiRerankResponse.RerankResult> results = rerankModel.rerank("Test query", List.of("Test document"));
