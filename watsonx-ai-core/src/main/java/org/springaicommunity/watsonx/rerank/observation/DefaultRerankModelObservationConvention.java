...
public class DefaultRerankModelObservationConvention implements RerankModelObservationConvention {
	...
	private static final KeyValue REQUEST_MODEL_NONE = KeyValue
		.of(RerankModelObservationDocumentation.LowCardinalityKeyNames.REQUEST_MODEL, KeyValue.NONE_VALUE);

	private static final KeyValue RESPONSE_MODEL_NONE = KeyValue
		.of(RerankModelObservationDocumentation.LowCardinalityKeyNames.RESPONSE_MODEL, KeyValue.NONE_VALUE);
	...
	protected KeyValue aiOperationType(RerankModelObservationContext context) {
		return KeyValue.of(RerankModelObservationDocumentation.LowCardinalityKeyNames.AI_OPERATION_TYPE,
				context.getOperationMetadata().operationType());
	}

	protected KeyValue aiProvider(RerankModelObservationContext context) {
		return KeyValue.of(RerankModelObservationDocumentation.LowCardinalityKeyNames.AI_PROVIDER,
	...
	protected KeyValue requestModel(RerankModelObservationContext context) {
		return Optional.ofNullable(context.getOptions())
			.map(options -> options.getModel())
			.filter(StringUtils::hasText)
			.map(model -> KeyValue.of(RerankModelObservationDocumentation.LowCardinalityKeyNames.REQUEST_MODEL, model))
	...
	protected KeyValue responseModel(RerankModelObservationContext context) {
		return Optional.ofNullable(context.getResponse())
			...
			.filter(StringUtils::hasText)
			.map(model -> KeyValue.of(RerankModelObservationDocumentation.LowCardinalityKeyNames.RESPONSE_MODEL, model))
	...
	protected KeyValues documentCount(KeyValues keyValues, RerankModelObservationContext context) {
		return keyValues.and(RerankModelObservationDocumentation.HighCardinalityKeyNames.DOCUMENT_COUNT.asString(),
	...
	protected KeyValues topN(KeyValues keyValues, RerankModelObservationContext context) {
		return Optional.ofNullable(context.getOptions())
			...
			.map(topN -> keyValues.and(RerankModelObservationDocumentation.HighCardinalityKeyNames.TOP_N.asString(),
					String.valueOf(topN)))
			.orElse(keyValues);
	}
	...
	protected KeyValues inputTokenCount(KeyValues keyValues, RerankModelObservationContext context) {
		return Optional.ofNullable(context.getResponse())
			...
			.map(inputTokenCount -> keyValues.and(
					RerankModelObservationDocumentation.HighCardinalityKeyNames.USAGE_INPUT_TOKENS.asString(),
