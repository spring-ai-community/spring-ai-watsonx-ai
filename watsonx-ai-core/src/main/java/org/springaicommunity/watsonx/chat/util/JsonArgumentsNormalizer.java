...
/**
 * Utility class for normalizing JSON arguments from different model formats. Different
 * models (Mistral, Granite, etc.) format JSON tool call arguments differently, and this
 * utility ensures consistent parsing across all formats.
 *
	...
	public static String normalize(String jsonArgs) {
		...
		if (normalized.startsWith("\"") && normalized.endsWith("\"") && normalized.length() > 2) {
			...
			if (normalized.contains("\\\"") || normalized.contains("\\n")) {
				...
				normalized = normalized.replace("\\\\", "\u0000") // Temporary placeholder
																	// for escaped
																	// backslash
					.replace("\\\"", "\"")
					.replace("\\n", "\n")
